...
func LinuxDoBind(c *gin.Context) {
	if !common.LinuxDOOAuthEnabled {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": common.LanguageString("administrator has not enabled Linux DO login and registration", "管理员未开启通过 Linux DO 登录以及注册"),
		})
		return
	}
	...
	if model.IsLinuxDOIdAlreadyTaken(user.LinuxDOId) {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": common.LanguageString("this Linux DO account is already linked", "该 Linux DO 账户已被绑定"),
		})
		return
	}
	...
	if model.IsLinuxDOIdAlreadyTaken(user.LinuxDOId) {
		...
		if common.RegisterEnabled {
			if linuxdoUser.TrustLevel >= common.LinuxDOMinimumTrustLevel {
				...
				c.JSON(http.StatusOK, gin.H{
					"success": false,
					"message": common.LanguageString("Linux DO trust level does not meet the minimum trust level set by the administrator", "Linux DO 信任等级未达到管理员设置的最低信任等级"),
				})
				return
			}
		} else {
