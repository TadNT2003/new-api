...
func (*CreemAdaptor) RequestPay(c *gin.Context, req *CreemPayRequest) {
	if req.PaymentMethod != model.PaymentMethodCreem {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": common.LanguageString("unsupported payment channel", "不支持的支付渠道")})
		return
	}

	if req.ProductId == "" {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": common.LanguageString("please select a product", "请选择产品")})
		return
	}
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem product config parsing failed user_id=%d error=%q", c.GetInt("id"), err.Error()))
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": common.LanguageString("product configuration error", "产品配置错误")})
		return
	}
	...
	if selectedProduct == nil {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": common.LanguageString("product does not exist", "产品不存在")})
		return
	}
	...
	err = topUp.Insert()
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem top-up order creation failed user_id=%d trade_no=%s product_id=%s error=%q", id, referenceId, selectedProduct.ProductId, err.Error()))
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem payment link creation failed user_id=%d trade_no=%s product_id=%s error=%q", id, referenceId, selectedProduct.ProductId, err.Error()))
...
func CreemWebhook(c *gin.Context) {
	...
	bodyBytes, err := io.ReadAll(c.Request.Body)
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem webhook request body read failed path=%q client_ip=%s error=%q", c.Request.RequestURI, c.ClientIP(), err.Error()))
	...
	if signature == "" {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Creem webhook signature missing path=%q client_ip=%s body=%q", c.Request.RequestURI, c.ClientIP(), string(bodyBytes)))
...
func handleCheckoutCompleted(c *gin.Context, event *CreemWebhookEvent) {
	...
	if event.Object.Order.Status != "paid" {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem order not paid, skipping request_id=%s order_id=%s order_status=%s", event.Object.RequestId, event.Object.Order.Id, event.Object.Order.Status))
		c.Status(http.StatusOK)
		return
	}
	...
	if event.Object.Order.Type != "onetime" {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem order type not yet supported, skipping request_id=%s creem_order_id=%s order_type=%s", referenceId, event.Object.Order.Id, event.Object.Order.Type))
		c.Status(http.StatusOK)
		return
	}
	...
	if topUp.Status != common.TopUpStatusPending {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem top-up order status is not pending, skipping trade_no=%s status=%s creem_order_id=%s", referenceId, topUp.Status, event.Object.Order.Id))
...
func genCreemLink(ctx context.Context, referenceId string, product *CreemProduct, email string, username string) (string, error) {
	if setting.CreemApiKey == "" {
		return "", fmt.Errorf(common.LanguageString("Creem API key not configured", "未配置Creem API密钥"))
	}
	...
	if err != nil {
		return "", fmt.Errorf(common.LanguageString("failed to serialize request data: %v", "序列化请求数据失败: %v"), err)
	}

	// 创建 HTTP 请求
	req, err := http.NewRequest("POST", apiUrl, bytes.NewBuffer(jsonData))
	if err != nil {
		return "", fmt.Errorf(common.LanguageString("failed to create HTTP request: %v", "创建HTTP请求失败: %v"), err)
	}
	...
	if err != nil {
		return "", fmt.Errorf(common.LanguageString("failed to send HTTP request: %v", "发送HTTP请求失败: %v"), err)
	}
	...
	if err != nil {
		return "", fmt.Errorf(common.LanguageString("failed to read response: %v", "读取响应失败: %v"), err)
	}
	...
	if err != nil {
		return "", fmt.Errorf(common.LanguageString("failed to parse response: %v", "解析响应失败: %v"), err)
	}
