...
func (*StripeAdaptor) RequestAmount(c *gin.Context, req *StripePayRequest) {
	if req.Amount < getStripeMinTopup() {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": fmt.Sprintf(common.LanguageString("top-up amount cannot be less than %d", "充值数量不能小于 %d"), getStripeMinTopup())})
		return
	}
	id := c.GetInt("id")
	...
	if payMoney <= 0.01 {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": common.LanguageString("top-up amount is too low", "充值金额过低")})
		return
	}
...
func (*StripeAdaptor) RequestPay(c *gin.Context, req *StripePayRequest) {
	...
	if req.Amount < getStripeMinTopup() {
		c.JSON(http.StatusOK, gin.H{"message": fmt.Sprintf(common.LanguageString("top-up amount cannot be less than %d", "充值数量不能小于 %d"), getStripeMinTopup()), "data": 10})
		return
	}
	if req.Amount > 10000 {
		c.JSON(http.StatusOK, gin.H{"message": common.LanguageString("top-up amount cannot exceed 10000", "充值数量不能大于 10000"), "data": 10})
		return
	}

	if req.SuccessURL != "" && common.ValidateRedirectURL(req.SuccessURL) != nil {
		c.JSON(http.StatusBadRequest, gin.H{"message": common.LanguageString("payment success redirect URL is not in the trusted domain list", "支付成功重定向URL不在可信任域名列表中"), "data": ""})
		return
	}

	if req.CancelURL != "" && common.ValidateRedirectURL(req.CancelURL) != nil {
		c.JSON(http.StatusBadRequest, gin.H{"message": common.LanguageString("payment cancel redirect URL is not in the trusted domain list", "支付取消重定向URL不在可信任域名列表中"), "data": ""})
		return
	}
...
func StripeWebhook(c *gin.Context) {
	...
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("Stripe webhook request body read failed path=%q client_ip=%s error=%q", c.Request.RequestURI, c.ClientIP(), err.Error()))
	...
	callerIp := c.ClientIP()
	logger.LogInfo(ctx, fmt.Sprintf("Stripe webhook signature verification successful event_type=%s client_ip=%s path=%q", string(event.Type), callerIp, c.Request.RequestURI))
	switch event.Type {
...
func sessionCompleted(ctx context.Context, event stripe.Event, callerIp string) {
	...
	if "complete" != status {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe checkout.completed status abnormal, skipping trade_no=%s status=%s client_ip=%s", referenceId, status, callerIp))
		return
	}
...
func sessionAsyncPaymentFailed(ctx context.Context, event stripe.Event, callerIp string) {
	...
	if len(referenceId) == 0 {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe async payment failure event missing order number client_ip=%s", callerIp))
		return
	}
	...
	if topUp.Status != common.TopUpStatusPending {
		logger.LogInfo(ctx, fmt.Sprintf("Stripe async payment failed but order not in pending status, skipping trade_no=%s status=%s client_ip=%s", referenceId, topUp.Status, callerIp))
		return
	}
...
func sessionExpired(ctx context.Context, event stripe.Event) {
	...
	status := event.GetObjectValue("status")
	if "expired" != status {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe checkout.expired status abnormal, skipping trade_no=%s status=%s", referenceId, status))
		return
	}

	if len(referenceId) == 0 {
		logger.LogWarn(ctx, common.LanguageString("Stripe checkout.expired missing order number", "Stripe checkout.expired 缺少订单号"))
		return
	}
	...
	logger.LogInfo(ctx, fmt.Sprintf("Stripe top-up order marked as expired trade_no=%s", referenceId))
}
...
func genStripeLink(referenceId string, customerId string, email string, amount int64, successURL string, cancelURL string) (string, error) {
	if !strings.HasPrefix(setting.StripeApiSecret, "sk_") && !strings.HasPrefix(setting.StripeApiSecret, "rk_") {
		return "", fmt.Errorf(common.LanguageString("invalid Stripe API key", "无效的Stripe API密钥"))
	}
