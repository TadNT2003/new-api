...
func SyncUpstreamModels(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusOK, gin.H{"success": false, "message": common.LanguageString("failed to get model list, please try again later", "获取模型列表失败，请稍后重试")})
		return
	}
	...
	wg.Wait()
	if fetchErr != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": common.LanguageString("failed to get upstream models: ", "获取上游模型失败: ") + fetchErr.Error(), "locale": req.Locale, "source_urls": gin.H{"models_url": modelsURL, "vendors_url": vendorsURL}})
		return
	}
...
func SyncUpstreamPreview(c *gin.Context) {
	...
	wg.Wait()
	if fetchErr != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": common.LanguageString("failed to get upstream models: ", "获取上游模型失败: ") + fetchErr.Error(), "locale": locale, "source_urls": gin.H{"models_url": modelsURL, "vendors_url": vendorsURL}})
		return
	}
