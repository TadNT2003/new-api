...
func UpdateMidjourneyTaskBulk() {
	...
	for {
		...
		logger.LogInfo(ctx, fmt.Sprintf(common.LanguageString("detected unfinished tasks: %v", "检测到未完成的任务数有: %v"), len(tasks)))
		...
		for channelId, taskIds := range taskChannelM {
			...
			if err != nil {
				...
				err := model.MjBulkUpdate(taskIds, map[string]any{
					"fail_reason": fmt.Sprintf(common.LanguageString("failed to get channel info, please contact the administrator, channel ID: %d", "获取渠道信息失败，请联系管理员，渠道ID：%d"), channelId),
					"status":      "FAILURE",
					"progress":    "100%",
				})
				if err != nil {
			...
			for _, responseItem := range responseItems {
				...
				if useTime > 3600000 && task.Progress != "100%" {
					responseItem.FailReason = common.LanguageString("upstream task timed out (exceeded 1 hour)", "上游任务超时（超过1小时）")
				...
				if responseItem.VideoUrls != nil && len(responseItem.VideoUrls) > 0 {
					...
					if err != nil {
						logger.LogError(ctx, fmt.Sprintf(common.LanguageString("failed to serialize VideoUrls: %v", "序列化 VideoUrls 失败: %v"), err))
				...
				if (task.Progress != "100%" && responseItem.FailReason != "") || (task.Progress == "100%" && task.Status == "FAILURE") {
					logger.LogInfo(ctx, task.MjId+common.LanguageString(" build failed: ", " 构建失败，")+task.FailReason)
					task.Progress = "100%"
					if task.Quota != 0 {
				...
				if err != nil {
					...
					model.RecordTaskBillingLog(model.RecordTaskBillingLogParams{
						...
						Other: map[string]interface{}{
							"task_id": task.MjId,
						"reason": common.LanguageString("image generation failed", "构图失败"),
						},
					})
				}
			}
