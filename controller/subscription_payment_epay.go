...
func SubscriptionRequestEpay(c *gin.Context) {
	...
	if plan.PriceAmount < 0.01 {
		common.ApiErrorMsg(c, common.LanguageString("plan amount is too low", "套餐金额过低"))
		return
	}
	if !operation_setting.ContainsPayMethod(req.PaymentMethod) {
		common.ApiErrorMsg(c, common.LanguageString("payment method does not exist", "支付方式不存在"))
		return
	}
	...
	if err != nil {
		common.ApiErrorMsg(c, common.LanguageString("callback address configuration error", "回调地址配置错误"))
		return
	}
	...
	if client == nil {
		common.ApiErrorMsg(c, common.LanguageString("payment information not configured by administrator", "当前管理员未配置支付信息"))
		return
	}
	...
	if err := order.Insert(); err != nil {
		common.ApiErrorMsg(c, common.LanguageString("order creation failed", "创建订单失败"))
		return
	}
	...
	if err != nil {
		...
		common.ApiErrorMsg(c, common.LanguageString("payment initiation failed", "拉起支付失败"))
		return
	}
