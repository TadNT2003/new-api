...
func validateChannel(channel *model.Channel, isAdd bool) error {
	...
	if channel.Type == constant.ChannelTypeVertexAi {
		...
		if err != nil {
			...
			return fmt.Errorf("deployment region must be valid JSON format, e.g. {\"default\": \"us-central1\", \"region2\": \"us-east1\"}")
		}
...
func getVertexArrayKeys(keys string) ([]string, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("batch add Vertex AI must use valid JSON array format, e.g. [{key1}, {key2}...], please check input: %w", err)
	}
	...
	if len(cleanKeys) == 0 {
		...
		return nil, fmt.Errorf("Vertex AI batch add keys cannot be empty")
	}
...
func EditTagChannels(c *gin.Context) {
	...
	if channelTag.ParamOverride != nil {
		...
		if trimmed != "" && !json.Valid([]byte(trimmed)) {
			...
			"message": "parameter override must be valid JSON format",
			})
			return
		}
	...
	if channelTag.HeaderOverride != nil {
		...
		if trimmed != "" && !json.Valid([]byte(trimmed)) {
			...
			"message": "request header override must be valid JSON format",
			})
			return
		}
	...
	case "disable_key":
		if request.KeyIndex == nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				// "message": "未指定要禁用的密钥索引",
			"message": "key index to disable not specified",
			})
			return
		}
	...
	case "enable_key":
		if request.KeyIndex == nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				// "message": "未指定要启用的密钥索引",
			"message": "key index to enable not specified",
			})
			return
		}
	...
	case "delete_key":
		if request.KeyIndex == nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				// "message": "未指定要删除的密钥索引",
			"message": "key index to delete not specified",
			})
			return
		}
