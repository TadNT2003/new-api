...
func RequestWaffoAmount(c *gin.Context) {
	...
	if req.Amount < waffoMinTopup {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": fmt.Sprintf(common.LanguageString("top-up amount cannot be less than %d", "充值数量不能小于 %d"), waffoMinTopup)})
		return
	}
...
func RequestWaffoPay(c *gin.Context) {
	if !setting.WaffoEnabled {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": common.LanguageString("Waffo payment not enabled", "Waffo 支付未启用")})
		return
	}
	...
	if req.PayMethodIndex != nil {
		...
		if idx < 0 || idx >= len(methods) {
			...
			c.JSON(http.StatusOK, gin.H{"message": "error", "data": common.LanguageString("payment method not supported", "不支持的支付方式")})
			return
		}
	...
	if err := topUp.Insert(); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo top-up order creation failed user_id=%d trade_no=%s amount=%d error=%q", id, merchantOrderId, req.Amount, err.Error()))
	...
	if err != nil {
		...
		_ = topUp.Update()
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": common.LanguageString("payment configuration error", "支付配置错误")})
		return
	}
	...
	if !resp.IsSuccess() {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo order business creation failed user_id=%d trade_no=%s code=%s message=%q response=%q", id, merchantOrderId, resp.Code, resp.Message, common.GetJsonString(resp)))
		topUp.Status = common.TopUpStatusFailed
		_ = topUp.Update()
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": common.LanguageString("payment initiation failed", "拉起支付失败")})
		return
...
func WaffoWebhook(c *gin.Context) {
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo webhook request body read failed path=%q client_ip=%s error=%q", c.Request.RequestURI, c.ClientIP(), err.Error()))
...
func handleWaffoPayment(c *gin.Context, wh *core.WebhookHandler, result *core.PaymentNotificationResult) {
	if result.OrderStatus != "PAY_SUCCESS" {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo order status not successful, skipping top-up trade_no=%s order_status=%s client_ip=%s", result.MerchantOrderID, result.OrderStatus, c.ClientIP()))
