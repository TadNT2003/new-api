...
func ValidateNumericCode(code string) (string, error) {
	...
	if len(code) != 6 {
		return "", fmt.Errorf(LanguageString("verification code must be 6 digits", "验证码必须是6位数字"))
	}

	// 检查是否为纯数字
	if _, err := strconv.Atoi(code); err != nil {
		return "", fmt.Errorf(LanguageString("verification code can only contain digits", "验证码只能包含数字"))
	}
