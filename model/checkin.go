...
func UserCheckin(userId int) (*Checkin, error) {
	...
	if !setting.Enabled {
		return nil, errors.New(common.LanguageString("check-in feature is not enabled", "签到功能未启用"))
	}
	...
	if hasChecked {
		return nil, errors.New(common.LanguageString("already checked in today", "今日已签到"))
	}
...
func userCheckinWithTransaction(checkin *Checkin, userId int, quotaAwarded int) (*Checkin, error) {
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Create(checkin).Error; err != nil {
			return errors.New(common.LanguageString("check-in failed, please try again later", "签到失败，请稍后重试"))
		}

		// 步骤2: 在事务中增加用户额度
		if err := tx.Model(&User{}).Where("id = ?", userId).
			Update("quota", gorm.Expr("quota + ?", quotaAwarded)).Error; err != nil {
			return errors.New(common.LanguageString("check-in failed: error updating quota", "签到失败：更新额度出错"))
		}
...
func userCheckinWithoutTransaction(checkin *Checkin, userId int, quotaAwarded int) (*Checkin, error) {
	...
	if err := DB.Create(checkin).Error; err != nil {
		return nil, errors.New(common.LanguageString("check-in failed, please try again later", "签到失败，请稍后重试"))
	}
	...
	if err := IncreaseUserQuota(userId, quotaAwarded, true); err != nil {
		// 如果增加额度失败，需要回滚签到记录
		DB.Delete(checkin)
		return nil, errors.New(common.LanguageString("check-in failed: error updating quota", "签到失败：更新额度出错"))
	}
