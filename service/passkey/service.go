...
func BuildWebAuthn(r *http.Request) (*webauthn.WebAuthn, error) {
	...
	if settings == nil {
		return nil, errors.New(common.LanguageString("Passkey settings not found", "未找到 Passkey 设置"))
	}
	...
	if originsStr != "" {
		...
		for _, origin := range originList {
			...
			if !settings.AllowInsecureOrigin && strings.HasPrefix(strings.ToLower(trimmed), "http://") {
				return nil, fmt.Errorf(common.LanguageString("Passkey does not allow insecure Origin: %s", "Passkey 不允许使用不安全的 Origin: %s"), trimmed)
			}
...
autoDetect:
	...
	if scheme == "http" && !settings.AllowInsecureOrigin && r.Host != "localhost" && r.Host != "127.0.0.1" && !strings.HasPrefix(r.Host, "127.0.0.1:") && !strings.HasPrefix(r.Host, "localhost:") {
		return nil, fmt.Errorf(common.LanguageString("Passkey only supports HTTPS, current access: %s://%s, please allow insecure Origin in Passkey settings or configure HTTPS", "Passkey 仅支持 HTTPS，当前访问: %s://%s，请在 Passkey 设置中允许不安全 Origin 或配置 HTTPS"), scheme, r.Host)
	}
	// 优先使用请求的完整Host（包含端口）
	host := r.Host
	...
	if host == "" {
		return nil, fmt.Errorf(common.LanguageString("cannot determine Passkey Origin, please specify in system or Passkey settings. Current Host: '%s', ServerAddress: '%s'", "无法确定 Passkey 的 Origin，请在系统设置或 Passkey 设置中指定。当前 Host: '%s', ServerAddress: '%s'"), r.Host, system_setting.ServerAddress)
	}
	if scheme == "" {
		scheme = "https"
	}
...
func resolveRPID(r *http.Request, settings *system_setting.PasskeySettings, origins []string) (string, error) {
	...
	if len(origins) == 0 {
		return "", errors.New(common.LanguageString("Passkey Origin not configured, cannot derive RPID", "Passkey 未配置 Origin，无法推导 RPID"))
	}
	parsed, err := url.Parse(origins[0])
	if err != nil {
		return "", fmt.Errorf(common.LanguageString("failed to parse Passkey Origin: %w", "无法解析 Passkey Origin: %w"), err)
	}
