...
autoDetect:
	...
	if host == "" {
		...
		return nil, fmt.Errorf("cannot determine Passkey Origin, please specify in system or Passkey settings. Current Host: '%s', ServerAddress: '%s'", r.Host, system_setting.ServerAddress)
	}
	if scheme == "" {
		scheme = "https"
	}
