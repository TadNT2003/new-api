...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	...
	if summary.WebSearchCallCount > 0 {
		// extraContent = append(extraContent, fmt.Sprintf("Web Search 调用 %d 次，调用花费 %s", summary.WebSearchCallCount, ...))
	...
	if summary.ClaudeWebSearchCallCount > 0 {
		// extraContent = append(extraContent, fmt.Sprintf("Claude Web Search 调用 %d 次，调用花费 %s", summary.ClaudeWebSearchCallCount, ...))
	...
	if summary.FileSearchCallCount > 0 {
		// extraContent = append(extraContent, fmt.Sprintf("File Search 调用 %d 次，调用花费 %s", summary.FileSearchCallCount, ...))
	...
	if summary.AudioInputPrice > 0 && summary.AudioTokens > 0 {
		// extraContent = append(extraContent, fmt.Sprintf("Audio Input 花费 %s", ...))
	...
	if summary.ImageGenerationCallPrice > 0 {
		// extraContent = append(extraContent, fmt.Sprintf("Image Generation Call 花费 %s", ...))
	...
	if summary.TotalTokens == 0 {
		...
		extraContent = append(extraContent, "upstream returned no billing info, cannot deduct quota (possibly upstream timeout)")
