...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	originUsage := usage
	if usage == nil {
		extraContent = append(extraContent, common.LanguageString("no billing info from upstream", "上游无计费信息"))
	}
	if originUsage != nil {
	...
	if summary.WebSearchCallCount > 0 {
		extraContent = append(extraContent, fmt.Sprintf(common.LanguageString("Web Search calls: %d, cost: %s", "Web Search 调用 %d 次，调用花费 %s"), summary.WebSearchCallCount, decimal.NewFromFloat(summary.WebSearchPrice).Mul(decimal.NewFromInt(int64(summary.WebSearchCallCount))).Div(decimal.NewFromInt(1000)).Mul(decimal.NewFromFloat(summary.GroupRatio)).Mul(decimal.NewFromFloat(common.QuotaPerUnit)).String()))
	}
	if summary.ClaudeWebSearchCallCount > 0 {
		extraContent = append(extraContent, fmt.Sprintf(common.LanguageString("Claude Web Search calls: %d, cost: %s", "Claude Web Search 调用 %d 次，调用花费 %s"), summary.ClaudeWebSearchCallCount, decimal.NewFromFloat(summary.ClaudeWebSearchPrice).Div(decimal.NewFromInt(1000)).Mul(decimal.NewFromFloat(summary.GroupRatio)).Mul(decimal.NewFromFloat(common.QuotaPerUnit)).Mul(decimal.NewFromInt(int64(summary.ClaudeWebSearchCallCount))).String()))
	}
	if summary.FileSearchCallCount > 0 {
		extraContent = append(extraContent, fmt.Sprintf(common.LanguageString("File Search calls: %d, cost: %s", "File Search 调用 %d 次，调用花费 %s"), summary.FileSearchCallCount, decimal.NewFromFloat(summary.FileSearchPrice).Mul(decimal.NewFromInt(int64(summary.FileSearchCallCount))).Div(decimal.NewFromInt(1000)).Mul(decimal.NewFromFloat(summary.GroupRatio)).Mul(decimal.NewFromFloat(common.QuotaPerUnit)).String()))
	}
	if summary.AudioInputPrice > 0 && summary.AudioTokens > 0 {
		extraContent = append(extraContent, fmt.Sprintf(common.LanguageString("Audio Input cost: %s", "Audio Input 花费 %s"), decimal.NewFromFloat(summary.AudioInputPrice).Div(decimal.NewFromInt(1000000)).Mul(decimal.NewFromInt(int64(summary.AudioTokens))).Mul(decimal.NewFromFloat(summary.GroupRatio)).Mul(decimal.NewFromFloat(common.QuotaPerUnit)).String()))
	}
	if summary.ImageGenerationCallPrice > 0 {
		extraContent = append(extraContent, fmt.Sprintf(common.LanguageString("Image Generation Call cost: %s", "Image Generation Call 花费 %s"), decimal.NewFromFloat(summary.ImageGenerationCallPrice).Mul(decimal.NewFromFloat(summary.GroupRatio)).Mul(decimal.NewFromFloat(common.QuotaPerUnit)).String()))
	}

	if summary.TotalTokens == 0 {
		extraContent = append(extraContent, common.LanguageString("upstream returned no billing info, cannot deduct quota (possibly upstream timeout)", "上游没有返回计费信息，无法扣费（可能是上游超时）"))
	...
	if strings.HasPrefix(logModel, "gpt-4-gizmo") {
		...
		extraContent = append(extraContent, fmt.Sprintf(common.LanguageString("model %s", "模型 %s"), summary.ModelName))
	}
